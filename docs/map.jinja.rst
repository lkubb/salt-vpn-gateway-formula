...
We will see a quick setup to configure the ``vpngw`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``vpngw`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``vpngw`` formulas under ``/srv/salt/vpngw/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/vpngw/parameters/dns:domain/
    ln -s dns:domain /srv/salt/vpngw/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/vpngw/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/vpngw/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``vpngw`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``vpngw`` formulas under ``/srv/salt/vpngw/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/vpngw/parameters/roles
...
We create a configuration for the role ``vpngw/server`` in ``/srv/salt/vpngw/parameters/roles/vpngw/server.yaml``:
...
We create another configuration for the role ``vpngw/client`` in ``/srv/salt/vpngw/parameters/roles/vpngw/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles vpngw/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles vpngw/client
...
.. code-block:: console
    ...
    mv /srv/salt/vpngw/parameters/dns:domain/  '/srv/salt/vpngw/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-vpngw-as-json:
      test.nop:
        - name: {{ vpngw | json }}
...
Here is an example based on `template-formula/vpngw/config/file.sls`_:
    ...
    vpngw-config-file-file-managed:
      file.managed:
        ...
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='vpngw-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ vpngw.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``vpngw`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ vpngw['winner'] }}
...
.. _template-formula/vpngw/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/vpngw/config/file.sls
